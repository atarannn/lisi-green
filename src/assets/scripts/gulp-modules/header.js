...
function menuOpen(menu) {
  ...
  tl.fromTo(menu, { x: 2000 }, { x: 0, duration: 0.4 }, '<');
  tl.play();
...
function menuClose(menu) {
  ...
  const tl = gsap.timeline({ paused: true });
  tl.fromTo(menu, { autoAlpha: 1, x: 0 }, { x: 2000, duration: 0.4, clearProps: 'all' }, '<');
...
document.addEventListener('DOMContentLoaded', function() {
  var consentBanner = document.getElementById('cookies');
  var acceptCookiesBtn = document.getElementById('accept-cookie');
